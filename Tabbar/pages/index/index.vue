...
export default {
	...
	onPullDownRefresh: function() {
		// 触发下拉刷新时执行
		console.log('index 页面 onPullDownRefresh', Date.now());
	},
	onReachBottom: function() {
		// 页面触底时执行
		console.log('index 页面 onReachBottom', Date.now());
	},
	onShareAppMessage: function() {
		// 页面被用户分享时执行
		console.log('index 页面 onShareAppMessage', Date.now());
	},
	onPageScroll: function() {
		// 页面滚动时执行
		console.log('index 页面 onPageScroll', Date.now());
	},
	watch: {
		tabbar: {
			handler(val) {
				console.log('index 页面 watch', Date.now());
			},
			deep: true,
			immediate: true
		}
